...
set "regFile=HKCU_GoldenState.reg"
...
echo   para este usuario. Aguarde...
echo.
echo.
...
if not exist "%~dp0\%regFile%" (
    echo   [ERRO] Arquivo de Registro ausente: %regFile%
    set "error=1"
) else (
...
echo.
echo [ACAO]   Aplicando modo furtivo (ocultando arquivos)...
attrib +s +h "%secretPath%\*" >nul 2>&1
attrib +s +h "%secretPath%" >nul 2>&1
echo [SUCESSO] Modo furtivo ativado.
echo.
echo   +----------------------------------------------------------+
echo   ^|                                                          ^|
